    ...
    >
      <nav className="lg:w-11/12 2xl:w-4/5 w-full md:px-6 2xl:px-0 mx-auto py-4 hidden sm:flex items-center justify-between">
        ...
        <ul className="flex items-center gap-8">
          ...
          <span
            ...
            className="hover:bg-gray-100 hover:dark:bg-cyan-600 p-1.5 rounded-full cursor-pointer transition-colors"
          >
      ...
      <nav className="p-4 flex sm:hidden items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <span
            onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
            className="bg-gray-100 dark:bg-cyan-600 p-1.5 rounded-full cursor-pointer transition-colors"
          >
            {theme === "dark" ? <FiSun /> : <FiMoon />}
          </span>
      ...
      >
        ...
        <div className="flex flex-col p-4 gap-5 bg-gray-100/95 backdrop-filter backdrop-blur-sm dark:bg-grey-900/95 w-3/4">
          ...
          {navs.slice(0, 4).map((e) => (
            <ScrollLink
              key={e}
              className="hover:text-cyan-600 py-1.5 px-4 rounded transition-colors capitalize cursor-pointer"
              to={e}
              offset={-60}
              smooth={true}
              duration={500}
          ...
          <ScrollLink
            ...
            duration={500}
            onClick={() => setNavCollapse(true)}
            className="px-6 py-1.5 rounded-md bg-cyan-600 hover:bg-cyan-600 text-white text-center"
          >
            Contact
          </ScrollLink>
        </div>
